...
### KoreanSearch
TIDAL은 한국에서 공식적으로 서비스되지 않기 때문에, 한국에서 발매된 음악이 TIDAL에서는 영어 제목으로 표시되는경우가 많습니다. 이 플러그인은 검색어에 한글이 포함되어 있는지 감지하고, 다른 곳에서 해당 곡의 영어 제목을 자동으로 찾아 TIDAL에서 검색한 뒤, 일치하는 결과를 TIDAL의 일반 검색 결과보다 위에 표시합니다

주요 기능:
- Apple Music (iTunes Search API)를 이용하여 제목 현지화 후 검색 (미국/한국이 trackId를 공유함)
- MusicBrainz 녹음 검색으로 한국어 제목 -> 라틴 문자 제목 / 아티스트 별칭 / ISRC 확보
- 못 찾으면 KOMCA(한국음악저작권협회) 저작물 검색을 사용. 부제목에 영문 제목이 등록되어 있습니다
- ISRC로 찾은 곡을 우선합니다. 표기 언어와 무관하게 정확히 같은 녹음

(모든 과정에서 API키는 필요없습니다)
